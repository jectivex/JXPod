...
class MonitoringScriptLoader: JXScriptLoader {
    ...
    public init(updateInterval: TimeInterval = 0.2, log: @escaping (String) -> Void = { print($0) }) {
    ...
    public func loadScript(from url: URL) throws -> String? {
        ...
        print("Loading JavaScript at \(sourceURL)")
    ...
    @discardableResult private func monitor(url: URL, sendingUpdateOf updateURL: URL, isUpdated: Bool = false) -> Bool {
        ...
        guard handle != -1 else {
            print("Unable to monitor JavaScript at \(url)")
            return false
        }
...
private class SourceFileLocator {
    init() {
        ...
        if let sourceDirectories = env["JXSOURCEPATH"]?.split(separator: ":").map({ URL(fileURLWithPath: String($0), isDirectory: true) }) {
            ...
            print("JavaScript dynamic reload source path: \(sourceDirectories)")
        } else if let projectDirectory = Self.findProjectDirectory(env) {
            print("JavaScript dynamic reload source path: \(projectDirectory)")
    ...
    func sourceFile(for url: URL) -> URL? {
        ...
        guard url.pathExtension == "js" else {
            print("WARNING: JavaScript file \(url) should have a .js extension for dynamic reload support")
            return url
        }
